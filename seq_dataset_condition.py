...
class DataProcess:
    ...
    def load_data(self, paths, mode='train'):
        ...
        :return:
        '''
        if 'carla_manual_20220509' in self.paths[0]:
            folder_name_list = [1, 2, 3, 4, 5,]
            time_scale = 2
            old_index = 0
            indices = []
            for idx, each_folder_name in enumerate(folder_name_list):
                ...
                # 1
                path = os.path.join(paths[0], '{}'.format(each_folder_name))
                ...
                if self.baseline == '1_NN_1_img_no_GCN':
                    ...
                    for view_idx_start in range(0, len(frames_np), 4):
                        temporal1_img = np.concatenate((frames_np[view_idx_start + 0], frames_np[view_idx_start + 1]),
                                                       axis=1)
                        temporal2_img = np.concatenate((frames_np[view_idx_start + 2], frames_np[view_idx_start + 3]),
                                                       axis=1)
                    ...
                    new_data = np.zeros((frames_np.shape[0] // self.num_views, self.image_width, self.image_width,
                                         self.img_channel * self.num_views))
                    for i in range(self.num_views):
                        new_data[:, :, :, self.img_channel * i:self.img_channel * (i + 1)] = data[i::self.num_views][
                                                                                             :frames_np.shape[
                                                                                                  0] // self.num_views]
                ...
                if idx == 0:
                    ...
                    final_data = np.concatenate([final_data, final_data[:int(20 /time_scale)],
                                                 data_1[int(20 / time_scale):]], axis=0)
            ...
            # left
            path = os.path.join(paths[0], 'left')
            print('begin load data ' + str(path))
            ...
            for c_dir in c_dir_list:
                ...
                for file in p_c_dir_list:
                    frame_im = Image.open(os.path.join(c_dir_path, file))
                    frame_np = np.array(frame_im)[:, :, :3]  # (1000, 1000) numpy array
                    frames_np.append(frame_np)
                    frames_file_name.append(file)
            if self.baseline == '1_NN_1_img_no_GCN':
                frames_np_4to1 = []
            ...
            # right
            path = os.path.join(paths[0], 'right')
            print('begin load data ' + str(path))
            ...
            indices = []
            right_label = []
            index = len(data) + len(left_data) - 1
            # print('2 len(data):', len(data))
            while index >= self.seq_len - 1 + len(left_data):
                indices.append(index - self.seq_len + 1)
                # print(index - self.seq_len + 1)
                index -= 1
                right_label.append(1)
            right_data = data
            right_indices = indices[::-1]
            # data = np.concatenate([left_data, right_data], axis=0)
            data = np.concatenate([left_data, left_data[:80], right_data[80:]], axis=0)
