...
class DataProcess:
    ...
    def load_data(self, paths, mode='train'):
        ...
        for idx, each_folder_name in enumerate(folder_name_list):
            ...
            new_data = np.zeros((frames_np_array.shape[0] // self.num_views, self.image_width, self.image_width,
                                 self.img_channel * self.num_views))
            for i in range(self.num_views):
                new_data[:, :, :, self.img_channel * i:self.img_channel * (i + 1)] = data[i::self.num_views][
                                                                                     :frames_np_array.shape[
                                                                                          0] // self.num_views]
            ...
            new_data_t2nd = np.zeros((t2nd_frames_np_array.shape[0] // self.num_views, self.image_width, self.image_width,
                                 1 * self.num_views))
            for i in range(self.num_views):
                new_data_t2nd[:, :, :, 1 * i : (1 * (i+1))] = t2nd_data[i::1][:t2nd_frames_np_array.shape[0] // self.num_views]
            ...
            if idx == 0:
                ...
                final_data_t2nd = np.concatenate([final_data_t2nd, final_data_t2nd[:int(20 / time_scale)],
                                             data_1_t2nd[int(20 / time_scale):]], axis=0)
