...
def img2video_2_views_training_set(src_path='carla_town02_2_view_20220828_icra', target_path='', dataset_name='carla_town02_2_view_20220828_icra', factor=3):
    epoch_path = os.path.join(src_path)
    all_test_case = sorted(os.listdir(epoch_path))
    frame_list = []
    all_temporal1 = sorted(os.listdir(os.path.join(epoch_path, all_test_case[0])))
    # all_temporal2 = ['gt_{0:02d}_00.png'.format(i) for i in range(1, 10+1)] + ['pd_{0:02d}_00.png'.format(i) for i in range(10+1, 20+1)]
    for temporal1 in all_temporal1:
        temporal1_img_list = []
        for test_case in all_test_case:
            test_case_path = os.path.join(epoch_path, test_case, temporal1)
            print('Loading img from {}.'.format(test_case_path))
            BGRimage = cv2.imread(test_case_path)
            BGRimage = cv2.resize(BGRimage, (int(128 * factor), int(128 * factor)))
            RGBimage = cv2.cvtColor(BGRimage, cv2.COLOR_BGR2RGB)
            temporal1_img_list.append(RGBimage)
        temporal1_img = np.concatenate([temporal1_img_list[0], np.ones_like(temporal1_img_list[0][:, :4]), temporal1_img_list[1]], axis=1)
        # temporal2_img_list = []
        # for test_case in all_test_case:
        #     test_case_path = os.path.join(epoch_path, test_case, temporal1)
        #     print('Loading img from {}.'.format(test_case_path))
        #     BGRimage = cv2.imread(test_case_path)
        #     BGRimage = cv2.resize(BGRimage, (int(128 * 2), int(128 * 2)))
        #     RGBimage = cv2.cvtColor(BGRimage, cv2.COLOR_BGR2RGB)
        #     temporal2_img_list.append(RGBimage)
        # temporal2_img = np.concatenate(temporal2_img_list, axis=1)
        # diff_img = cv2.absdiff(temporal2_img, temporal1_img)
        frame = np.concatenate([temporal1_img], axis=0)
        # frame = np.concatenate([temporal1_img, np.ones_like(temporal1_img)[:4],
        #                         temporal2_img, np.ones_like(temporal1_img)[:4], diff_img], axis=0)
        frame_list.append(frame)

    clip = mpy.ImageSequenceClip(frame_list, fps=8)
    clip.write_videofile(os.path.join(target_path, 'vis_{}.mp4'.format(dataset_name)), fps=8)

def img2video_4_views_training_set(src_path='carla_town02_4_view_20220828_icra', target_path='', dataset_name='carla_town02_4_view_20221126_icra', factor=3):
    epoch_path = os.path.join(src_path)
    all_test_case = sorted(os.listdir(epoch_path))
    frame_list = []
    all_temporal1 = sorted(os.listdir(os.path.join(epoch_path, all_test_case[0])))
    ...
    for temporal1 in all_temporal1:
        ...
        for test_case in all_test_case:
            ...
            temporal1_img_list.append(RGBimage)
        temporal1_img = np.concatenate([temporal1_img_list[0], np.ones_like(temporal1_img_list[0][:, :4]), temporal1_img_list[1]], axis=1)
        temporal2_img = np.concatenate(
            [temporal1_img_list[2], np.ones_like(temporal1_img_list[0][:, :4]), temporal1_img_list[3]], axis=1)
        # temporal2_img_list = []
        # for test_case in all_test_case:
        #     test_case_path = os.path.join(epoch_path, test_case, temporal1)
        #     print('Loading img from {}.'.format(test_case_path))
        ...
        # diff_img = cv2.absdiff(temporal2_img, temporal1_img)
        # frame = np.concatenate([temporal1_img], axis=0)
        frame = np.concatenate([temporal2_img, np.ones_like(temporal1_img)[:4],
                                temporal1_img], axis=0)
        frame_list.append(frame)
...
def astar_img2video_single(src_path='', target_path='', bs='', prefix='', fps=8):
    epoch_path = os.path.join(src_path)
    all_test_case = sorted(os.listdir(epoch_path))[-1000:]
    frame_list = []
    # all_temporal1 = ['gt_{0:02d}_00.png'.format(i) for i in range(1, 20+1)]
    ...
    clip.write_videofile(os.path.join(target_path, 'astar_{}_{}.mp4'.format(prefix, bs)), fps=fps)

def img2video_single(src_path='/home/jsun/data/Programs/msl-traffic-prediction/packages/carla_tools/save_img_dir/', target_path='', bs='', prefix='', fps=1):
    epoch_path = os.path.join(src_path)
    all_test_case = sorted([i for i in os.listdir(epoch_path) if 'pred_frame_01_000000_' in i])
    if len(all_test_case) > 0:
        frame_list = []
        for test_case in all_test_case:
            test_case_path = os.path.join(epoch_path, test_case)
            print('Loading img from {}.'.format(test_case_path))
        ...
        clip = mpy.ImageSequenceClip(frame_list, fps=fps)
        clip.write_videofile(os.path.join(target_path, '{}_{}.mp4'.format(prefix, bs)), fps=fps)
...
def img2video_multiple_types_2_views(src_path='', target_path='', num_views=1, fps=8, history_length=10, add_grid=True):
    empty_H = 210
    empty_W = 400
    template_array = np.ones((empty_H, empty_W, 3))  # (79,394,3)
    img_idx = 0
    grid_img = None
    ...
    while os.path.exists(os.path.join(src_path, 't2no_{0:06d}.png'.format(img_idx))):
        # and os.path.exists(os.path.join(src_path, 'rgb_{0:06d}.png'.format(img_idx))) and \
        #         os.path.exists(os.path.join(src_path, 't2no_wo_ego_{0:06d}.png'.format(img_idx))) and os.path.exists(os.path.join(src_path, 'grid_img_{0:06d}.png'.format(img_idx)))
        # RGB
        # try:
        tmp_list = []
        # t2no
        ...
        view_test_case_path = os.path.join(src_path, 'all_view_{0:06d}.png'.format(img_idx))
        print('Loading img from {}'.format(view_test_case_path))
        all_view_img = cv2.resize(cv2.cvtColor(cv2.imread(view_test_case_path), cv2.COLOR_BGR2RGB), (int(128 * 2), int(128)))  # * 255 / history_length
        all_view_img_empty = np.ones((min(all_view_img.shape[0], empty_H), max(all_view_img.shape[1], empty_W), 3))
        all_view_img_empty[:int(all_view_img.shape[0]), :int(all_view_img.shape[1])] = all_view_img
        # print(all_view_img_empty.shape, empty_array.shape)
        empty_array = np.concatenate((empty_array, all_view_img_empty), axis=0)

        if add_grid:
            view_test_case_path = os.path.join(src_path, 'grid_img_{0:06d}.png'.format(img_idx))
            print('Loading img from {}'.format(view_test_case_path))
            if os.path.exists(os.path.join(src_path, 'grid_img_{0:06d}.png'.format(img_idx))):
                grid_img = cv2.imread(view_test_case_path)  # * 255 / history_length
            elif grid_img is None:
                grid_img = np.ones((128, 128, 3))
            else:
                print('Skip {}'.format(os.path.join(src_path, 'grid_img_{0:06d}.png'.format(img_idx))))
            grid_img_empty = np.ones((min(grid_img.shape[0], empty_H), max(grid_img.shape[1], empty_W), 3))
            grid_img_empty[:int(grid_img.shape[0]), :int(grid_img.shape[1])] = grid_img
            # print(grid_img_empty.shape, empty_array.shape)
            empty_array = np.concatenate((empty_array, grid_img_empty), axis=0)
...
def img2video_multiple_types_4_views(src_path='', target_path='', num_views=1, fps=8, history_length=10, add_grid=True):
    empty_H = 210
    empty_W = 400
    template_array = np.ones((empty_H, empty_W, 3))  # (79,394,3)
    img_idx = 0
    ...
    while os.path.exists(os.path.join(src_path, 't2no_{0:06d}.png'.format(img_idx))):
        ...
        view_test_case_path = os.path.join(src_path, 'all_view_{0:06d}.png'.format(img_idx))
        print('Loading img from {}'.format(view_test_case_path))
        all_view_img = cv2.resize(cv2.cvtColor(cv2.imread(view_test_case_path), cv2.COLOR_BGR2RGB), (int(128 * 2), int(128/2)))  # * 255 / history_length
        all_view_img_empty = np.ones((min(all_view_img.shape[0], empty_H), max(all_view_img.shape[1], empty_W), 3))
        all_view_img_empty[:int(all_view_img.shape[0]), :int(all_view_img.shape[1])] = all_view_img
        # print(all_view_img_empty.shape, empty_array.shape)
        empty_array = np.concatenate((empty_array, all_view_img_empty), axis=0)
...
if __name__ == "__main__":
    ...
    '''
    img2video_single(target_path='/media/data/jack/Programs/msl-traffic-prediction/packages/carla_tools')
    try:
        astar_img2video_single(src_path='/media/data/jack/Programs/msl-traffic-prediction/packages/carla_tools/save_img_dir_grid',
            target_path='/media/data/jack/Programs/msl-traffic-prediction/packages/carla_tools', prefix='grid')
    except:
        pass
    # astar_img2video_single(
    #     src_path='/media/data/jack/Programs/msl-traffic-prediction/packages/carla_tools/save_img_dir_minkowski',
    #     target_path='/media/data/jack/Programs/msl-traffic-prediction/packages/carla_tools', prefix='minkowski')

    # img2video_multiple_types_2_views(
    #     src_path='/media/data/jack/Programs/msl-traffic-prediction/packages/carla_tools/save_img_dir',
    #     target_path='/media/data/jack/Programs/msl-traffic-prediction/packages/carla_tools')

    img2video_multiple_types_4_views(src_path='/media/data/jack/Programs/msl-traffic-prediction/packages/carla_tools/save_img_dir',
                                     target_path='/media/data/jack/Programs/msl-traffic-prediction/packages/carla_tools')
    ...
    for bs_name in bs_name_list:
        ...
        '''
        # img2video_2_views_training_set()
        # img2video_4_views_training_set(src_path='/home/jsun/data/Programs/msl-traffic-prediction/packages/carla_tools/carla_town02_4_view_20221126_icra',
        #                             target_path='/home/jsun/data/Programs/msl-traffic-prediction/packages/carla_tools/carla_town02_4_view_20221126_icra')