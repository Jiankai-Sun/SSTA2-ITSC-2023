...
class DataProcess:
    ...
    def load_data(self, paths, mode='train'):
        ...
        # top left
        path = os.path.join(paths[0], '_out_2')
        print('begin load data ' + str(path))
        ...
        for i in range(len(frames_np)):
            temp = np.float32(frames_np[i])
            data[i, :, :, :] = cv2.resize(temp, (self.image_width, self.image_width)) / 255
        top_left_data = copy.copy(data)
        del data

        # top right right
        path = os.path.join(paths[0], '_out_3')
        print('begin load data ' + str(path))
        ...
        data = np.concatenate([left_data, right_data, top_left_data, top_right_data], axis=-1)
