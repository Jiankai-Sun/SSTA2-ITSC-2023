...
def main():
    ...
    try:
        ...
        for n, transform in enumerate(spawn_points):
            ...
            # actor = SpawnActor(blueprint, transform)
            # print('actor.id: ', actor.id)
            # actor.then(SetAutopilot(FutureActor, True))
        ...
        for i, each_vehicle in enumerate(vehicles):
            if each_vehicle.id in [228]:  # , 261
                vehicle_id_228 = each_vehicle
                print(i, 228)
            elif each_vehicle.id in [261]:  # , 261
                vehicle_id_261 = each_vehicle
                print(i, 261)

        agent = BasicAgent(vehicle_id_228)
        spawn_point = world.get_map().get_spawn_points()[0]
        # spawn_point = world.map.get_spawn_points()[80]
